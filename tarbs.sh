#!/bin/sh
# tenklo's Auto Rice Boostrapping Script (tARBS)
...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://github.com/tenklo/dotfiles.git"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/tenklo/tARBS/master/progs.csv"
...
welcomemsg() { \
	dialog --title "Welcome!" --msgbox "Welcome to tenklo's Auto-Rice Bootstrapping Script!\\n\\nThis script will automatically install a fully-featured Linux desktop, which I use as my main machine.\\n\\n-tenklo" 10 60
...
gitmakeinstall() {
	...
	dialog --title "tARBS Installation" --infobox "Installing \`$progname\` ($n of $total) via \`git\` and \`make\`. $(basename "$1") $2" 5 70
...
for x in curl base-devel git ntp zsh; do
	dialog --title "tARBS Installation" --infobox "Installing \`$x\` which is required to install and configure other programs." 5 70
	installpkg "$x"
done

dialog --title "tARBS Installation" --infobox "Synchronizing system time to ensure successful and secure installation of software..." 4 70
...
dialog --title "tARBS Installation" --infobox "Finally, installing \`libxft-bgra\` to enable color emoji in suckless software without crashes." 5 70
...
https://www.youtube.com/feeds/videos.xml?channel_id=UC2eYFnH61tmytImy1mTYvhA \"~tenklo Smith (YouTube)\"
...
%wheel ALL=(ALL) NOPASSWD: /usr/bin/shutdown,/usr/bin/reboot,/usr/bin/systemctl suspend,/usr/bin/wifi-menu,/usr/bin/mount,/usr/bin/umount,/usr/bin/pacman -Syu,/usr/bin/pacman -Syyu,/usr/bin/packer -Syu,/usr/bin/packer -Syyu,/usr/bin/systemctl restart NetworkManager,/usr/bin/rc-service NetworkManager restart,/usr/bin/pacman -Syyu --noconfirm,/usr/bin/loadkeys,/usr/bin/yay,/usr/bin/pacman -Syyuw --noconfirm,/usr/bin/make clean install,/usr/bin/make install"
